...
app.get("/recepie/:id/edit", (req, res) => {
    ...
    const { id, title, text, img } = req.query;
    recepies.map({ id, title, text, img });
...
//create new recepie POST
app.post("/recepies", (req, res) => { console.log(req.body); });



//delete route

app.delete("/recepie/:id/delete", (req, res) => {
    let id = req.params.id

    const product = async(id) => await recepies.destroy({ where: { id: id } })
    ...
    res.render("recepies/delete", { foundRecepie });
    res.send("Successfully Deleted the item");
});
// app.post("/", (req, res) => {
//     let id = req.params.id;
